...
#load data
df = pd.read_csv('../data/movies_genres.csv', index_col='movieid')

pca = PCA()
components = pca.fit_transform(df.drop(['title','year'], axis = 1))
...
X = df.drop(['title','year'], axis = 1)
loadings = pca.components_[0:2].T * np.sqrt(pca.explained_variance_[0:2])
fig3 = px.scatter(components, x=0, y=1)
for i, X.columns in enumerate(X.columns):
    fig3.add_shape(
        type='line',
        x0=0, y0=0,
        x1=loadings[i, 0],
